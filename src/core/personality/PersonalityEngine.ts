...
export type PersonalityMode = 'normal' | 'samimi' | 'profesyonel' | 'koc' | 'antrenor' | 'yonetici';

export interface EmotionalContext {
  state: 'sad' | 'tired' | 'frustrated' | null;
  addon: string;
}

const PERSONALITY_PROMPTS: Record<PersonalityMode, string> = {
  normal:
    'Sen Moni adında akıllı, samimi ve profesyonel bir yapay zeka asistanısın. ' +
  ...
  samimi:
    'Sen Moni adında sıcakkanlı, enerjik ve arkadaş canlısı bir yapay zeka asistanısın. ' +
  ...
  profesyonel:
    'Sen Moni adında kurumsal, ciddi ve profesyonel bir yapay zeka asistanısın. ' +
  ...
  koc:
    'Sen Moni adında motive edici, ilham veren ve yol gösteren bir yaşam koçu asistanısın. ' +
  ...
  antrenor:
    'Sen Moni adında disiplinli, motive edici ve enerjik bir spor antrenörü asistanısın. ' +
  ...
  yonetici:
    'Sen Moni adında stratejik düşünen, liderlik vasıflarına sahip bir üst düzey yönetici asistanısın. ' +
...
const MODE_SWITCH_PATTERNS: { pattern: RegExp; mode: PersonalityMode }[] = [
...
const EMOTIONAL_PATTERNS: { pattern: RegExp; state: 'sad' | 'tired' | 'frustrated' }[] = [
...
const EMOTIONAL_ADDONS: Record<'sad' | 'tired' | 'frustrated', string> = {
  sad:
    'Kullanıcı şu an üzgün veya morali bozuk. Destekleyici, anlayışlı ve empatik ol. ' +
    'Onu dinlediğini hissettir. Gerekirse hafif bir plan veya moral veren bir öneri sun. ' +
    'Asla yargılama, sadece yanında ol.',
  tired:
    'Kullanıcı şu an yorgun veya enerjisi düşük. Anlayışlı ol, fazla talepkar olma. ' +
    'Dinlenmesini öner veya günü kolaylaştıracak basit öneriler sun. Hafif ve rahatlatıcı konuş.',
  frustrated:
    'Kullanıcı şu an sinirli veya stresli. Sakin, anlayışlı ve yapıcı ol. ' +
...
export class PersonalityEngine {
   ...
   */
  public getSystemPrompt(userName?: string, emotionalContext?: EmotionalContext | null): string {
   ...
   */
  public detectModeSwitch(text: string): PersonalityMode | null {
   ...
   */
  public detectEmotionalState(text: string): EmotionalContext | null {
    const cleanText = text.toLowerCase().trim();
   ...
   */
  public static getModeLabel(mode: PersonalityMode): string {
    const labels: Record<PersonalityMode, string> = {
      normal: 'Normal',
      samimi: 'Samimi',
   ...
   */
  public getModeSwitchConfirmation(mode: PersonalityMode, userName?: string): string {
    const name = userName || '';
    const namePrefix = name ? `${name}, ` : '';

    const confirmations: Record<PersonalityMode, string> = {
  ...
  private loadMode(): PersonalityMode {
    try {
      if (typeof window !== 'undefined' && window.localStorage) {
        const stored = localStorage.getItem(STORAGE_KEY);
        if (stored && this.isValidMode(stored)) {
          return stored as PersonalityMode;
        }
      }
    } catch (_) {
  ...
  private saveMode(mode: PersonalityMode): void {
    try {
      if (typeof window !== 'undefined' && window.localStorage) {
