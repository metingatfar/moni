...
export interface MemoryFact extends MemoryItem {}

export class LongTermMemory {
  private facts: MemoryItem[] = [];
  
  // Diagnostics metrics
  private duplicateBlockedCount = 0;
  private lastSavedFact: MemoryItem | null = null;
  private lastUsedFact: MemoryItem | null = null;
  private status: 'idle' | 'initializing' | 'ready' = 'idle';

  constructor() {
    this.initialize();
    
    // Subscribe to database changes to keep in-memory cache in sync
    eventBus.subscribe('MemorySaved', (memory: any) => this.handleExternalSave(memory));
    eventBus.subscribe('MemoryUpdated', (memory: any) => this.handleExternalUpdate(memory));
    eventBus.subscribe('MemoryDeleted', (payload: any) => this.handleExternalDelete(payload));
  }
   ...
   */
  public mapLegacyMemoryItem(item: any): MemoryItem {
    ...
    let category = item.category as any;
   ...
   */
  public getSimilarityScore(str1: string, str2: string): number {
  ...
  public async addFact(
    ...
    confidence = 1.0, 
    source: 'explicit' | 'implicit' | 'manual' = 'explicit',
    importance = 3
  ): Promise<{ success: boolean; action: 'saved' | 'updated' | 'duplicate_blocked'; memory: MemoryItem }> {
   ...
   */
  public recordUsage(fact: MemoryItem): void {
  ...
  private handleExternalSave(memory: any): void {
  ...
  private handleExternalUpdate(memory: any): void {
    if (!memory || !memory.id) return;
    const mapped = this.mapLegacyMemoryItem(memory);
    this.facts = this.facts.map(f => f.id === mapped.id ? mapped : f);
  }

  private handleExternalDelete(payload: any): void {
